...
for i in range(len(BETAS)):
    NOMBRES.append(['Patchy_random_'+str(BETAS[i])+'.BSKIndex','Patchy_'+str(BETAS[i])+'.BSKIndex']) 
...
for i in range(len(NOMBRES)):
    ...
    #se analiza random
    puntos_ran=np.loadtxt('Patchy_random.txt',usecols=0)
    ...
    y_ran=P/total_ran
    #se analiza datos
    puntos_dat=np.loadtxt('Patchy.txt',usecols=0)
