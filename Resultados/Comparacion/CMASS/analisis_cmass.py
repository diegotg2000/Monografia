...
for i in range(len(BETAS)):
    NOMBRES.append(['data_random_'+str(BETAS[i])+'.BSKIndex','data_'+str(BETAS[i])+'.BSKIndex']) 
...
for i in range(len(NOMBRES)):
    ...
    #se analiza random
    puntos_ran=np.loadtxt('data_random.txt',usecols=0)
    ...
    y_ran=P/total_ran
    #se analiza datos
    puntos_dat=np.loadtxt('data.txt',usecols=0)
