...
for i in range(len(BETAS)):
    NOMBRES.append(['bMD_random_'+str(BETAS[i])+'.BSKIndex','bMD_'+str(BETAS[i])+'.BSKIndex']) 
...
for i in range(len(NOMBRES)):
    ...
    #se analiza random
    puntos_ran=np.loadtxt('bMD_random.txt',usecols=0)
    ...
    y_ran=P/total_ran
    #se analiza datos
    puntos_dat=np.loadtxt('bMD.txt',usecols=0)
